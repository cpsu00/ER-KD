...
from data_loader import get_tin_test_loader, get_tin_train_loader
from utils import accuracy, AverageMeter, LazyAverageMeterDict, print_and_write
import models


parser = argparse.ArgumentParser(description='FCFD tin experiment')
USE_FP16 = False
...
data_arg.add_argument('--num_classes', type=int, default=200,
                      help='Number of classes to classify')
data_arg.add_argument('--batch_size', type=int, default=64,
                      help='# of images in each batch of data')
data_arg.add_argument('--data_dir', type=str, default='../../data/tiny-imagenet-200',
...
train_arg.add_argument('--momentum', type=float, default=0.9,
                       help='Momentum value')
train_arg.add_argument('--epochs', type=int, default=240,
...
def main():
    ...
    test_data_loader = get_tin_test_loader(
    ...
    if config.is_train:
        ...
        train_data_loader = get_tin_train_loader(
...
class Trainer(object):
    ...
    def train(self):
        ...
        for i in range(self.model_num):
            # build models
            if i == 0:
                m = self.model_factory.__dict__[self.config.teacher](num_classes=200)
                print(f"creating model {i} (teacher): {self.config.teacher}")
            else:
                m = self.model_factory.__dict__[self.config.student](num_classes=200)
                print(f"creating model {i} (student): {self.config.student}")
            model_list.append(m)
            m.cuda()
            summary(m, input_size=[(3, 32, 32)], batch_size=1, device="cuda")

        config = self.config

        self.model = self.model_factory.MetaModel(model_list, extra_type=self.config.extra_type,)
        self.output_stages = self.model.output_stages
        self.model.cuda()

        self.model.load_teacher(torch.load(self.config.teacher_ckpt, map_location="cuda")['model'])

        self.optimizer = optim.SGD(self.model.parameters(), lr=config.init_lr, momentum=config.momentum,
                                   weight_decay=config.weight_decay, nesterov=config.nesterov)

        if self.config.scheduler == "step":
            if self.config.epochs == 240:
                mile = [150, 180, 210]
            elif self.config.epochs == 200:
                mile = [100, 150]
            elif self.config.epochs == 300:
                mile = [150, 225]
            else:
                raise ValueError
        ...
        if self.config.resume is not None:
            ...
            self.best_valid_accs = checkpoint['best_valid_acc']
        ...
        for epoch in range(start_epoch, self.config.epochs):
            ...
            for i in range(self.model_num):
                ...
                print('best_valid_accs: {}'.format(self.best_valid_accs[i]))
    ...
    def train_one_epoch(self, epoch):
        ...
        for iter_count, (images, labels) in enumerate(self.train_loader):
            ...
            # get entropy
            logits_teacher = outputs[0]
            p_t = F.softmax(logits_teacher / 4, dim=1)
            ...
            for stage in range(len(inters_all_stages)):
                ...
                for val in list(inters.values())[1:]:
                    loss_l2 += torch.mean(F.mse_loss(val, inter_target, reduction="none") * entropy.view(-1, 1, 1, 1))
                if is_final:
                ...
                if epoch < 20:
                    loss_l2 = min(1, (epoch+1) / 20) * loss_l2
                ...
                with torch.cuda.amp.autocast(USE_FP16):
                    ...
                    for j in range(len(outputs)):
                        if i != j:
                            ...
                            if weight != 0:
                                kl_loss_ij = (self._loss_kl(outputs[i], outputs[j] ,T) * entropy.unsqueeze(1)).sum(1).mean()
    ...
    def validate(self, epoch):
        self.model.eval()
        meter_accs_dict = LazyAverageMeterDict()
        ...
        for _, (images, labels) in enumerate(self.valid_loader):
            ...
            for i in range(len(outputs)):
                ...
                meter_accs_dict.update(full_paths[i], prec.item(), images.size()[0])

        with open(os.path.join(self.config.logs_dir, f"latest_result.log"), 'w') as f:
            for path_name, acc in meter_accs_dict.items():
                print_and_write(f, f"model {path_name}: acc: {acc.avg:.3f}")

        return list(meter_accs_dict.inner.values())
    ...
    def test(self):
        ...
        self.model = self.model_factory.__dict__[self.config.student](num_classes=200)
        self.model.eval()
        self.model.cuda()
        # load checkpoint
