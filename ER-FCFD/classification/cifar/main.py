...
class Trainer(object):
    ...
    def train(self):
        ...
        for i in range(self.model_num):
            # build models
            if i == 0:
                m = self.model_factory.__dict__[self.config.teacher](num_classes=100)
                print(f"creating model {i} (teacher): {self.config.teacher}")
            else:
                m = self.model_factory.__dict__[self.config.student](num_classes=100)
    ...
    def train_one_epoch(self, epoch):
        ...
        for iter_count, (images, labels) in enumerate(self.train_loader):
            ...
            p_t = F.softmax(logits_teacher / 3, dim=1)
    ...
    def test(self):
        ...
        self.model = self.model_factory.__dict__[self.config.student](num_classes=100)
        self.model.eval()
        self.model.cuda()
        # load checkpoint
