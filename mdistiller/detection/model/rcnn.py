...
from mdistiller.distillers.KD import kd_loss, er_kd_loss
...
def rcnn_erkd_loss(stu_predictions, tea_predictions, temperature, t_prime):
    stu_logits, _ = stu_predictions
    tea_logits, _ = tea_predictions
    loss_erkd = er_kd_loss(stu_logits, tea_logits, temperature, t_prime)
    return {
        'loss_erkd': loss_erkd,
    }
...
class RCNNKD(nn.Module):
    ...
    def forward(self, batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        if self.kd_args.TYPE == "ERKD":
            ...
            detector_losses.update(rcnn_erkd_loss(stu_predictions, tea_predictions, self.kd_args.ERKD.T, self.kd_args.ERKD.T_p))
        elif self.kd_args.TYPE == "DKD":
            teacher_images = self.teacher_preprocess_image(batched_inputs)
            t_features = self.teacher.backbone(teacher_images.tensor)
            stu_predictions = self.forward_pure_roi_head(self.roi_heads, features, sampled_proposals)
            tea_predictions = self.forward_pure_roi_head(self.teacher.roi_heads, t_features, sampled_proposals)
