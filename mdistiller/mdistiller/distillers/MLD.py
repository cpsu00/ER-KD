...
def er_kd_loss(logits_student, logits_teacher, temperature):
    ...
    loss = loss * entropy.unsqueeze(1)  # Expand entropy tensor to match the shape of loss tensor
...
class MLD(Distiller):
    def __init__(self, student, teacher, cfg, t, er):
        super(MLD, self).__init__(student, teacher)
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        if self.er == True:
            loss_kd_weak = self.kd_loss_weight * ((er_kd_loss(
                logits_student_weak,
                logits_teacher_weak,
                self.temperature,
                # reduce=False
            ) * mask).mean()) + self.kd_loss_weight * ((er_kd_loss(
                logits_student_weak,
                logits_teacher_weak,
                3.0,
                # reduce=False
            ...
            loss_kd_strong = self.kd_loss_weight * er_kd_loss(
                logits_student_strong,
                logits_teacher_strong,
                self.temperature,
            ) + self.kd_loss_weight * er_kd_loss(
                logits_student_strong,
                logits_teacher_strong,
                3.0,
            ) + self.kd_loss_weight * er_kd_loss(
