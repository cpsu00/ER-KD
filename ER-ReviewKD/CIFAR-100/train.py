...
def log(lr, epoch, log_dict, best_acc):
    # wandb log
    wandb.log({"current lr": lr})
    wandb.log(log_dict)
    wandb.run.summary["best_acc"] = float(best_acc)
...
from model.vgg import VGG, vgg13_bn, vgg8_bn
...
import os
tags = 'entropy,reviewkd,' + args.teacher + ',' + args.model
experiment_name = tags
tags = tags.split(",")
experiment_name = os.path.join("cifar100_baselines", experiment_name)
wandb.init(project="cifar100_baselines", name=experiment_name, tags=tags)
...
if 'x4' in args.teacher:
    ...
    teacher = vgg13_bn(num_classes=num_classes)
...
if teacher is not None:
    ...
    cnn = vgg8_bn(num_classes=num_classes)
...
for epoch in range(args.epochs):
    ...
    log(lr, epoch, log_dict, row['best_acc'])
