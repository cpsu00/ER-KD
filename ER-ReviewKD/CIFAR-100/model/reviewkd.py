...
from .vgg import VGG, vgg13_bn, vgg8_bn
...
def build_review_kd(model, num_classes, teacher = ''):
    ...
    if 'x4' in model: 
        ...
        student = vgg8_bn(num_classes=num_classes)
...
def hcl(fstudent, fteacher, entropy):
    loss_all = 0.0
    for fs, ft in zip(fstudent, fteacher):
        n,c,h,w = fs.shape
        # loss = F.mse_loss(fs, ft, reduction='mean')
        loss = torch.mean(F.mse_loss(fs, ft, reduction="none") * entropy.view(-1, 1, 1, 1))
        cnt = 1.0
        tot = 1.0
        for l in [4,2,1]:
            if l >=h:
            ...
            loss += torch.mean(F.mse_loss(tmpfs, tmpft, reduction="none") * entropy.view(-1, 1, 1, 1)) * cnt 
            tot += cnt
        loss = loss / tot
